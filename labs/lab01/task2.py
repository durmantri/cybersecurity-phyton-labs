...
users = {
    "red_team_lead": {"role": "red_team", "clearance": 4, "department": "RedTeam", "active": True},
...
resources = [("attack_scenarios", 4), ("defense_playbooks", 3),
...
def ewre(username: str) -> str:
    
    #заборонений доступ
    if username not in users:
        return "DENY (Usernot found)"
...
def main():
    ...
    for username in users:
        status = ewre(username)
        
        for resource_name, resource_level in resources:
            
            if status == "ALLOW":
                user_clearence = users[username]["clearance"] 
                
                if user_clearence >= resource_level:
                    final_status =  "ALLOW"
                else:
                    final_status = "DENY(Insufficient clearancef)"
            else:
                final_status = status
