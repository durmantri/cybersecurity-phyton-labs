...
passwords = ["InfoS3c@2023", "simple123", "Def3ns3@Key", "public", "Encrypt3d#Pass", "basic123",
              "Secur3@Analysis", "temp123", "Pr0t3ct@Data","default"]
...
def passwd_check(passwd: str, pass_counts: Counter) -> tuple[str, str]:
    ...
    #Дуже сильний
    if passed_count == 4 and len(passwd) >= criteria["min_length"] + 4 and pass_counts[passwd] == 1:
...
def main():
    ...
    table.add_column("Пароль", style="bold white")
    table.add_column("Надійність", justify="center")

    dublicated_passwd = set()

    for  passwd in passwords:
        if passwd in dublicated_passwd:
            continue
        dublicated_passwd.add(passwd)
